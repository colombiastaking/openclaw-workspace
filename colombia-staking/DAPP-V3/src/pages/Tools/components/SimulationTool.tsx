...
import styles from './SimulationTool.module.scss';
...
function simulateAprAndRank({
  ...
  for (const row of newStakers) {
    if (row.egldStaked > 0 && row.egldStaked > 0 && maxRatio !== minRatio) {
      row.normalized = (row.ratio - minRatio) / (maxRatio - minRatio);
    } else {
      row.normalized = 0;
    }
    row.aprBonus = APRmin + (bestAprMax - APRmin) * Math.sqrt(row.normalized || 0);
  }

  const sumColsStaked = newStakers.reduce((sum: number, row: any) => sum + (row.colsStaked || 0), 0);
  const totalEgldStaked = newStakers.reduce((sum: number, row: any) => sum + (row.egldStaked || 0), 0);
...
export const SimulationTool = ({ onBack }: Props) => {
